...
boolean pointInBox(Vec3 boxTopLeft, float boxW, float boxH, Vec3 pointPos){
  ...
  float margin = maxCircleRad+agentRad;
  if (diff.x > 0-margin && diff.x < boxW+margin && diff.y > 0-margin && diff.y < boxH+margin) return true;
  return false;
}
...
boolean pointInCircle(Vec3 center, float r, Vec3 pointPos){
  ...
  if (dist < r*4+agentRad){ //small safty factor
    return true;
  }
  return false;
}
...
boolean pointInCircleList(Vec3[] centers, float[] radii, int numObstacles, Vec3 pointPos){
  for (int i = 0; i < numObstacles; i++){
    ...
    //float r = radii[i];
    if (pointInCircle(center,maxCircleRad,pointPos)){
      return true;
    }
  }
  return false;
...
hitInfo rayCircleIntersect(Vec3 center, float r0, Vec3 l_start, Vec3 l_dir, float max_t){
  hitInfo hit = new hitInfo();
  
  //Step 2: Compute W - a displacement vector pointing from the start of the line segment to the center of the circle
    Vec3 toCircle = center.minus(l_start);
    float r = r0+agentRad+margin;
...
hitInfo rayCircleListIntersect(Vec3[] centers, float[] radii,  int numObstacles, Vec3 l_start, Vec3 l_dir, float max_t){
  ...
  for (int i = 0; i < numObstacles; i++){
    ...
    hitInfo boxHit = rayBoxIntersect(boxTopLeft, boxW, boxH,  l_start, l_dir, hit.t);
