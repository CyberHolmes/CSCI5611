...
void testPRM(){
  ...
  surface.setTitle(str2+"-"+str1+" pathLen="+nf(pathLength,0,1));
}
...
void draw(){
  ...
  if (curPath.size() == 1 && curPath.get(0) == -1) return;
  
  //Draw Planned Path
  stroke(20,255,40);
  strokeWeight(5);
  if (curPath.isEmpty()){ //direct path
...
void keyPressed(){
  ...
  if (key == '1'){ AStarEnable = true; BFSEnable = false; str2 = "AStar";}
  if (key == '2'){  AStarEnable = false; BFSEnable = true;str2 = "BFS";}
  if (key == '3'){ op1Enable = true; op2Enable = false; str1 = "Op1";}
  if (key == '4'){ op1Enable = false; op2Enable = true; str1 = "Op2";}
  //curPath = planPath(startPos, goalPos, circlePos, circleRad, numObstacles, nodePos, numNodes);
  printTestResults();
}
